...
Each figure is moved by eight MG90S metal/brass-geared micro servos, for a total of 16 on `CH00` through `CH15`. Every servo uses a three-pin harness: brown = ground, red = 5 V, and orange = PWM signal. The display stage is the lid of a 13 x 10 x 6.5 inch pine box. The metal-clasp wall with the 1/4 in power-entry hole is the front Service Edge / Service Wall; the opposite unclasped hinge wall is the back Audience Edge. When unlatched, the lid pivots forward toward the audience around that hinge edge. The pine-box setup is complete. The 8 x 10 x 1/4 inch MDF tray uses its bottom-left Service Wall corner as `(x=0, y=0)`; all zone boundaries, the 4 x 4 servo grid, and `CH00`–`CH15` labels have been hand-measured and penciled. The PCA9685 driver, barrel-to-screw-terminal block, and ESP32 are laid out in their Power & Controller Zone positions, with the ESP32 USB-C port facing left, but they still need to be fastened and no servos are mounted.

The 5 V / 15 A adapter remains external. Its red/black 14 AWG wires enter directly through the existing 1/4 in hole in the lower Service Wall, centered 2 in from the left corner and 1.5 in above the bottom edge. Because the hole is too small for a rubber grommet, electrical tape provides strain relief and chafing protection at the hole; inspect and replace that tape if it loosens or damages. In every top-down layout, the Service Wall is the bottom edge. The full-width tendon corridor is `y=0.0–1.5 in`; its open tendon-routing portion is `x=3.5–10.0 in`, because the 3.5 x 3.5 inch bottom-left Power & Controller Zone at `x=0.0–3.5 in`, `y=0.0–3.5 in` occupies the left-side baseline beside the power-entry hole. The servo field is a 6.5 x 6.5 inch grid at `x=3.5–10.0 in`, `y=1.5–8.0 in`: four rows and four columns of 1.625 x 1.625 inch cells, ordered `CH00`–`CH03`, `CH04`–`CH07`, `CH08`–`CH11`, and `CH12`–`CH15` from top to bottom. All output splines point down toward the bottom tendon corridor. Dupont jumpers carry only the ESP32-to-PCA9685 logic and I2C signals. Fasten each bare servo through its side tabs with its two supplied self-tapping wood screws; after electrical centering, secure the required double-arm horn with its supplied M2 machine screw and use it for the 20 lb braided PE tendon. The single-arm and cross horns remain spares.
...
**Raising the arms.** A clear T-shaped support is fixed to the lower box wall at the metal-clasp service edge, behind the actors from the audience-facing hinge edge when the lid is closed. It does not travel with the lid. Before using tensioned tendons, unplug 5 V power and open the lid fully to confirm that the moving stage, PTFE tubes, and service loops do not bind on the fixed support. Motors in the removable tray lift the arms naturally, and the clear support is designed to disappear under stage lighting.
