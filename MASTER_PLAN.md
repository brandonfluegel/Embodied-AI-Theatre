...
- **Service Edge / Service Wall** — the lid edge with the metal clasps and the matching lower-box wall behind it. Fix the clear acrylic T gantry to this lower-box wall, not to the lid. It sits behind the actors from the audience viewpoint when the lid is closed.
...
The **8 in x 10 in x 1/4 in MDF board** sits flat inside the box as a single removable internal tray. It carries the 16 MG90S servos, PCA9685, ESP32, barrel-to-screw-terminal block, and cable-management anchors. View the tray from the Service Edge / Service Wall when using the directional references below. Reserve these finalized areas:
...
- [ ] Build the removable 8 in × 10 in × 1/4 in MDF tray so it sits flat inside the box. From the Service Wall view, reserve a 1.5 in-wide horizontal tendon corridor along the clasp side, place the terminal block, PCA9685, and ESP32 in the bottom-left power/controller zone beside the left-side entry hole, and place the 16 MG90S servos in a `CH00`–`CH15` 4 x 4 field above and to the right. Orient the ESP32 USB-C port left toward the outer wall and every servo output spline toward the tendon corridor.
